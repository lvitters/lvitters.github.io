<script lang="ts">
	...
	// navigation tabs
	type View = 'info' | 'exhibitions' | 'impressum';
	let view = $state<View>('info');

	const menuItems: { id: View; label: string }[] = [
		{ id: 'info', label: 'info' },
		{ id: 'exhibitions', label: 'dates' },
		{ id: 'impressum', label: 'legal' }
	];
</script>
...
<div>
	...
	<main class="relative h-screen w-full overflow-x-hidden bg-white {mobile.current ? '' : 'flex'}">
